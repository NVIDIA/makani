...
def get_file_stats(filename, file_slice, bias=None, norm=None, batch_size=8, progress=None):
...
def get_file_histograms(
    filename, file_slice, minvals, maxvals, nbins, quadrature_weights, bias=None, norm=None, batch_size=8, progress=None
):
    ...
    with h5.File(filename, "r") as f:
        ...
        for batch_start in range(slc_start, slc_stop, batch_size):
            ...
            # generate histograms
            tmphistograms = [
                np.histogram(x, bins=nbins, range=(minval, maxval), weights=quadrature_weights)
...
def get_histograms(
    ...
    # quadrature:
    quadrature_weights = (
        GridQuadrature(quadrature_rule, (height, width), crop_shape=None, crop_offset=(0, 0), normalize=True)
        .quad_weight.cpu()
        .numpy()
    )
    ...
    for filename, index_bounds in mapping.items():
        tmpcount, tmpmins, tmpmaxs = get_file_stats(
            filename,
    ...
    print(
        f"Rank {comm_rank} stats done. Duration for {(samples_end - samples_start)} samples: {duration:.2f}s",
        flush=True,
    )
    comm.Barrier()
    ...
    histograms = None
    for filename, index_bounds in mapping.items():
        tmphistograms = get_file_histograms(
            filename,
            file_slice=slice(index_bounds[0], index_bounds[1] + 1),
            minvals=mins,
            maxvals=maxs,
    ...
    # wait for everybody else
    print(
        f"Rank {comm_rank} histograms done. Duration for {(samples_end - samples_start)} samples: {duration:.2f}s",
        flush=True,
    )
    comm.Barrier()
