    ...
    if comm_rank == 0:
        ...
        for sc in surface_channel_names_wb2:
            data_arrays[sc] = (
                ...
                da.zeros(
                    (total_entries, ensemble_size, lead_time, nlat, nlon),
                    chunks=(1, 1, lead_time, nlat, nlon),
                    dtype=dataset_dtype,
                ),
            )
        for ac in atmospheric_channel_names_wb2:
            data_arrays[ac] = (
                ...
                da.zeros(
                    (total_entries, ensemble_size, lead_time, nlevels, nlat, nlon),
                    chunks=(1, 1, lead_time, nlevels, nlat, nlon),
                    dtype=dataset_dtype,
                ),
            )
    ...
    for filename, ne in zip(file_names_to_convert, entries_per_year):
        ...
        with h5.File(filename, "r") as f:
            ...
            for ioff in range(ne_start, ne_end, local_batch_size):
                ...
                if verbose:
                    print(
                        f"{comm_rank}: file={filename}, ne={ne}, start={start}, end={end}, ne_start={ne_start}, ne_end={ne_end}, ne_local={ne_local}, local_batch_size={local_batch_size}, nsamples={nsamples}"
                    )
...
def main(args):
    ...
    convert(file_names_to_convert=files, output_file=args.output_file, batch_size=args.batch_size, verbose=args.verbose)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_file", type=str, help="Filename for saving wb2 compatible zarr file file.", required=True
    )
